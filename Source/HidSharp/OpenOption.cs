...
namespace HidSharp
{
    ...
    {
        static readonly Dictionary<Guid, OpenOption> _options;
        ...
        {
            Throw.If.Null(deserializeCallback, nameof(deserializeCallback));
            Throw.If.Null(serializeCallback, nameof(serializeCallback));

            OpenOption option;
            lock (_options)
            {
                if (_options.ContainsKey(guid)) { throw new ArgumentException("GUID keys are not supported."); }
